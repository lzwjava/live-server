...
class WxPayApi
{
     ...
     * sendRedPacket
     *
     * @param WxPayGroupRedPacket $inputObj
     * @throws WxPayException
     * @return result
     */
    public static function sendGroupRedPacket($inputObj, $timeOut = 6)
    {
        if (!$inputObj->IsRemarkSet()) {
            throw new WxPayException("需要设置 Remark");
        }
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            if (isDebug()) {
                curl_setopt($ch, CURLOPT_SSLCERT, WxPayConfig::SSLP12_PATH);
                curl_setopt($ch, CURLOPT_SSLCERTTYPE, "P12");
                curl_setopt($ch, CURLOPT_SSLKEYPASSWD, "abc");
            } else {
                //设置证书
