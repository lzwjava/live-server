...
func updateLiveAndSubmitThenPublish(c *Client, liveId string, amount int, needPay int) {
	...
	c.postData("lives/"+liveId, url.Values{"subject": {"C++ 编程"},
		"coverUrl":      {"https://i.quzhiboapp.com/34VRME"},
...
func TestLives_get(t *testing.T) {
	...
	assert.NotNil(t, live.Get("coursewareKey").Interface())
	...
	assert.NotNil(t, live.Get("webHlsUrl").Interface())
...
func TestLives_update_one(t *testing.T) {
	...
	res := c.postData("lives/"+liveId, url.Values{"subject": {"C++ 编程"},
                "coverUrl":      {"https://i.quzhiboapp.com/mXpQm9"},
		"coursewareKey": {"jMyBvR.pdf"},
		...
		"speakerIntro": {"我是悟空，热爱旅行，小众目的地爱好者，也是人文地理摄影师。「摆脱千篇一律的旅程，探索完全属于你自己的世界，去尝试遇见全新的事物，直到世界成为你生命里的一部分。」这是我的旅行哲学，是我读世界的方式。幸运的是至今我也如此实践着，设计飞机涂装，为联合国拍摄公益项目，在远东的堪察加半岛住上一段，去拉达克隐秘的赞斯卡山谷找一座悬崖上的寺庙，为了看一眼 K2 在巴基斯坦喀喇昆仑山区徒步两个星期……每段旅程对我来说都是全新的世界。"},
		"planTs":       {planTs}, "previewUrl": {"  "}, "needPay": {"1"},
	...
	assert.Equal(t, res.Get("coverUrl").MustString(), "//i.quzhiboapp.com/mXpQm9")
...
func TestLives_attended(t *testing.T) {
	...
	attendLives := c.getData("lives/attended", url.Values{"userId": {userId2}})
	assert.Equal(t, len(attendLives.MustArray()), 1)
	assert.Equal(t, liveId, toStr(attendLives.GetIndex(0).Get("liveId").MustInt()))
}
