...
/*        if (isset($data[KEY_COVER_URL])) {
    ...
    function updateTopic_post($liveId)
    {
        ...
        $op = $this->request->getPost(KEY_OP);
        if ($op == OP_ADD) {
            if (!$topicId) {
    ...
    public function recommend()
    {
        $skip = $this->skip();
        $limit = $this->limit();
        $skipLiveId = $this->request->getGet(KEY_SKIP_LIVE_ID);
        if (!$skipLiveId) {
            $skipLiveId = 0;
        }
        $user = $this->getSessionUser();
    ...
    function notifyLiveStartRecommend_get($liveId)
    {
        ...
        $relatedLiveId = $this->request->getGet('relatedLiveId');
        $live = $this->liveDao->getLive($liveId);
    ...
    function notifyLiveStart_get($liveId)
    {
        ...
        $oneHour = $this->toNumber($this->request->getGet('oneHour'));
        if ($oneHour) {
