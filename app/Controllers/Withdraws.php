...
class Withdraws extends BaseController
{
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getPost(), array(KEY_AMOUNT))) {
            return;
        }
    ...
    public function agree($withdrawId)
    {
        if ($this->checkIfNotAdmin()) {
            return;
        }
    ...
    function createByManual_post()
    {
        ...
        $amount = intval($this->request->getPost(KEY_AMOUNT));
        $userId = $this->request->getPost(KEY_USER_ID);
