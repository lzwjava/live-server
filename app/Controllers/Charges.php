...
class Charges extends BaseController
{
    ...
    public function one()
    {
        if ($this->checkIfParamsNotExist($this->request->getGet(), array(KEY_ORDER_NO))) {
            return;
        }
        $orderNo = $this->request->getGet(KEY_ORDER_NO);
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getPost(), array(KEY_ORDER_NO, KEY_REMARK))) {
            return;
        }
        $orderNo = $this->request->getPost(KEY_ORDER_NO);
        $remark = $this->request->getPost(KEY_REMARK);
    ...
    public function create()
    {
        ...
        if ($this->checkIfParamsNotExist($this->request->getPost(), array(KEY_CHANNEL, KEY_AMOUNT))) {
            return;
        }
        $channel = $this->request->getPost(KEY_CHANNEL);
        $amount = $this->toNumber($this->request->getPost(KEY_AMOUNT));
    ...
    function appleCallback_post()
    {
        ...
        $receipt = $this->request->getPost(KEY_RECEIPT);
        $orderNo = $this->request->getPost(KEY_ORDER_NO);
