...
class Packets extends BaseController
{
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getPost(), array(KEY_TOTAL_AMOUNT,
            KEY_CHANNEL, KEY_TOTAL_COUNT, KEY_WISHING))
        ) {
            return;
        }
        $totalAmount = $this->request->getPost(KEY_TOTAL_AMOUNT);
        $totalCount = $this->request->getPost(KEY_TOTAL_COUNT);
        $channel = $this->request->getPost(KEY_CHANNEL);
        $wishing = $this->request->getPost(KEY_WISHING);
