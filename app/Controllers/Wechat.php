...
class Wechat extends BaseController
{
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getGet(), array(KEY_URL))) {
            return;
        }
    ...
    public function callback()
    {
        ...
        logInfo("wechat valid get str: " . json_encode($this->request->getGet()));
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getGet(), array(KEY_TYPE))) {
            return;
        }
        $type = $this->request->getGet(KEY_TYPE);
        $liveId = $this->toNumber($this->request->getGet(KEY_LIVE_ID));
        $packetId = $this->request->getGet(KEY_PACKET_ID);
        $data = null;
        if ($type == 'live') {
    ...
    {
        if ($this->checkIfParamsNotExist($this->request->getPost(), array(KEY_CODE))) {
            return;
        }
        $code = $this->request->getPost(KEY_CODE);
    ...
    function registerByApp()
    {
        ...
        $iv = $this->request->getPost(KEY_IV);
        $encryptedData = $this->request->getPost(KEY_ENCRYPTED_DATA);
        $thirdSession = $this->request->getPost(KEY_THIRD_SESSION);
        ...
        $rawData = $this->request->getPost(KEY_RAW_DATA);
        $signature = $this->request->getPost(KEY_SIGNATURE);
